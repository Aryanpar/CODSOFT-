# CODSOFT Android App Development Internship

This repository contains Android applications developed as part of the **CodSoft Android App Development Internship**.
The projects demonstrate core Android development concepts including UI design, activity navigation, and basic functionality.

---

## 📱 Projects

### 1️⃣ Todo List App

A simple productivity application that allows users to manage daily tasks.

**Features**

* Add tasks
* Delete tasks
* Simple and clean user interface

**Download APK**
[Download TodoListApp](APK/TodoListApp.apk)

---

### 2️⃣ Alarm Clock App

An Android application that allows users to set alarms and reminders.

**Features**

* Set alarm for a specific time
* Alarm notification
* Easy-to-use interface

**Download APK**
[Download AlarmClockApp](APK/AlarmClockApp.apk)

---

### 3️⃣ Quiz App

An interactive quiz application that tests user knowledge through multiple-choice questions.

**Features**

* Multiple-choice questions
* Score calculation
* Displays results at the end of the quiz

**Download APK**
[Download Quiz App](APK/quiz_demo_app.apk)

---

## 🛠 Technologies Used

* Flutter
* java
* Dart
* canva[logo design]
* local db
* Android Studio
* XML Layouts
* Android SDK

---

## 📂 Repository Structure

CODSOFT-

* AlarmClockApp → Alarm Clock source code
* TodoListApp → Todo List source code
* quiz_demo_app → Quiz App source code
* APK → Installable application files

---

## 👨‍💻 Author

**Aryan Vankar**
Android App Development Intern – CodSoft
